...
it('can convert to basic nepali date with helper function toBS', function (string $date, string $expectedResult) {
    expect(toBS($date))
        ->toBe($expectedResult);
})->with([
    ['1996-04-22', '2053-01-10'],
...
it('can convert to nepali formatted result with helper function toBS', function (string $format, string $locale, string $expectedResult) {
    expect(toBS("1996-04-22", $format, $locale))
        ->toBe($expectedResult);
})->with([
    ['d F Y, l', 'np', '१० वैशाख २०५३, सोमबार'],
    ['d F Y, l', 'en', '10 Baisakh 2053, Monday'],
